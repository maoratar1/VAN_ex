...
SUPP_ERR = 2
MOVIE_LEN = 3450
K, M1, M2 = utills.read_cameras()


# == Functions == #
def create_pts_cloud(img1, img2):
    """
    Return points cloud of two images by:
        1. find key points matches in the two images
        2. reject key points by the rectified policy rejection
        3. apply triangulation
    :return numpy array of 3d points shape (Key point num) X 3
    """
    # Compute key points and matches
...
def find_supporters_4_frame(pair0_p3d_pts,
    ...
    :return: Supporters indexes from left0 matches coordinates
    """
...
def whole_movie(first_left_ex_mat=M1):
    ...
    # Find matches in pair0 with rectified test
    left0_kpts, left0_dsc, right0_kpts, pair0_matches, pair0_rec_matches_idx = read_and_rec_match(utills.IDX)
    print("Pair 0")
    for i in range(1, MOVIE_LEN - 1):
        if i % 100 == 0:
            print("Pair ", i)
        left1_kpts, left1_dsc, right1_kpts, pair1_matches, pair1_rec_matches_idx = read_and_rec_match(utills.IDX + i)
...
def compute_whole_movie_time(first_left_ex_mat=M1):
    ...
    T_arr = whole_movie(first_left_ex_mat)
...
def compare_left_cam_2d_trajectory_to_ground_truth(left_cameras_pos, left_cameras_pos_gt, alg_name, match_method, time):
    """
...
def compare_left_cam_2d_trajectory_to_ground_truth_params(left_cameras_pos, left_cameras_pos_gt, alg_name, match_method,
                                                          whole_time):
    """
    Compare the left cameras relative 2d positions to the ground truth
    """
...
# ===== Missions =====
def mission2_dot3():
    left0, right0 = utills.read_images(utills.IDX)
    left1, right1 = utills.read_images(utills.IDX + 1)
