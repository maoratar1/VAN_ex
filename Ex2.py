...
def rectified_stereo_pattern_rej(img1_kpts, img2_kpts, matches):  # Todo: Efficiency for further exercises
    ...
    :return: Inliers and outliers of the 2 images
    """
...
def draw_matches_with_rectified_stereo_pattern_rej(img1_inliers, img2_inliers,
...
def compare_triangulations_pts_values_avg(p3d_pts, cv_p3d_pts):
    ...
    :param p3d_pts: Our p3d points
    :param cv_p3d_pts: cv points
    """
    dist = np.linalg.norm(p3d_pts - cv_p3d_pts)
    print(f"Average of distances between matching points: {dist / len(p3d_pts)}")
...
def triangulation_on_sequence_images():
    ...
    for i in range(num_imgs):
        # Read the images
        img1, img2 = utills.read_images(utills.IDX + i)

        # Compute matching and inliers
        img1_kpts, _, img2_kpts, _, matches = utills.read_detect_and_match(img1, img2)
    ...
    ax.set_title(f"Triangulation over {num_imgs} images")
...
if __name__ == '__main__':
    # Read images
    img1, img2 = utills.read_images(utills.IDX)
    ...
    img1_inliers, img2_inliers, img1_outliers, img2_outliers = rectified_stereo_pattern_rej(img1_kpts, img2_kpts, matches)
    ...
    # Triangulate over sequence of images
