...
def find_features_in_consecutive_frames_whole_movie(first_left_ex_cam_mat=M1):
    ...
    for i in tqdm.tqdm(range(1, MOVIE_LEN)):
        ...
        relative_to_first_trans.append(trans)

    global_trans = convert_trans_from_rel_to_global(relative_to_first_trans)

    return consecutive_frame_features, inliers_percentage, global_trans, relative_to_first_trans
...
def find_features_in_consecutive_frames(left0_kpts, left0_dsc, right0_kpts,
   ...
   """
    ...
    pair0_p3d_pts = triangulate(K @ M1, K @ M2, left0_matches_coor,
                                       right0_matches_coor)
...
def get_feature_obj(matches, img1_kpts, img2_kpts):
    ...
    for match in matches:
        ...
        img2_x, img2_y = img2_kpts[img2_kpt_idx].pt[0], img2_kpts[img2_kpt_idx].pt[1]
        y = (img1_y + img2_y) / 2
        feature = Feature.Feature(img1_kpt_idx, img2_kpt_idx, img1_x, img2_x, y)
